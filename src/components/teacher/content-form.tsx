  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6 mb-8">
        <h2 className="text-2xl md:text-3xl font-serif italic text-editorial-black">Curriculum Modules</h2>
        <button 
          onClick={() => setIsAdding(!isAdding)}
          className="w-full sm:w-auto flex items-center justify-center space-x-3 bg-editorial-black text-editorial-cream px-8 py-4 text-[10px] font-black uppercase tracking-widest hover:bg-editorial-black/90 transition-all shadow-xl"
        >
      ...
      {isAdding && (
        <form onSubmit={handleAddLesson} className="p-6 md:p-12 border-2 border-editorial-black bg-white shadow-2xl animate-in fade-in slide-in-from-top-4 duration-500">
           ...
           <button 
              disabled={isLoading}
              className="mt-12 w-full h-16 md:h-20 bg-editorial-black text-editorial-cream flex items-center justify-center space-x-4 group overflow-hidden"
           >
              {isLoading ? (
      ...
      <div className="space-y-4">
        {lessons.length === 0 ? (
          <div className="p-12 md:p-20 border border-editorial-black/10 bg-white/50 text-center">
            <Layout className="h-10 w-10 md:h-12 md:w-12 text-editorial-black/5 mx-auto mb-6" />
            <p className="text-lg md:text-xl font-serif italic text-editorial-black/40">No modules in the current registry.</p>
          </div>
        ) : (
          lessons.map((lesson, idx) => (
            <div key={lesson.id} className="group p-4 md:p-8 border border-editorial-black/10 bg-white hover:border-editorial-black transition-all flex flex-col sm:flex-row items-start sm:items-center gap-6 sm:gap-6">
              <div className="flex items-center space-x-4 w-full sm:w-auto">
                <div className="text-[10px] font-black text-editorial-black/20 group-hover:text-editorial-black transition-colors shrink-0">
                  {String(idx + 1).padStart(2, '0')}
                </div>
                
                <div className="flex items-center space-x-4 flex-1">
                  <div className="h-10 w-10 bg-editorial-black/5 flex items-center justify-center text-editorial-black/40 shrink-0">
                  ...
                  <div className="flex flex-col min-w-0">
                      ...
                      <h4 className="text-base md:text-lg font-serif text-editorial-black line-clamp-1">{lesson.title}</h4>
                  </div>
                </div>
              </div>

              <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto sm:flex-1 space-x-4">
                 <div className="text-[10px] font-serif italic text-editorial-black/20">
